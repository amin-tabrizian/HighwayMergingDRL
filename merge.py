...
def run(training = True):
    ...
    while not done :
        ...
        state = nstate
        if not done:
            speeds.append(nstate[12])
            positions.append(nstate[0])
        if traci.simulation.getCollidingVehiclesIDList() != ():
            collision = 1
        score += reward
        step += 1
    ...
    return collision, positions, speeds, score
...
if __name__ == "__main__":
    ...
    agent = Agent(gamma=0.99, epsilon=0.00, batch_size=64, n_actions=3, eps_end=0.0,
                  input_dims=[18], lr=0.001, eps_dec=5e-5)
    ...
    for i in range(epoch):
        collision, positions, speeds, score = run(training)
        collisions += collision
        scores.append(score)
        avg_score = np.mean(scores[-100:])
        # np.save('speed1' + str(i+ 1 ) + '.npy', speeds)
        # np.save('position1' + str(i+ 1 ) + '.npy', positions)
